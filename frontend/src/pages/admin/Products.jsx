  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      if (file.size > 1024 * 1024) { // 1MB limit
        showAlert('info', 'Image size should be less than 1MB');
        return;
      }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {products.map((product) => (
            ...
            >
              {/* Product Image */}
              {product.image ? (
  <div className="relative w-full h-48 bg-white rounded-xl mb-4 overflow-hidden border-2 border-gray-200 p-2">
    <img
      src={product.image}
      alt={product.name}
